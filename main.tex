...
\begin{figure}[H]
    \centering
    \safeimage[0.65\textwidth]{umlclasse.drawio.png}{Diagramme de classes TAMKIN}{Diagramme de classes TAMKIN}
...
Le modèle conceptuel des données de TAMKIN met en évidence les entités principales et leurs associations fonctions : gestion des comptes et rôles, pilotage des modules pédagogiques, communautés collaboratives, échanges en temps réel et suivi des progrès.

Les entités les plus importantes sont :
\begin{itemize}
  \item \textbf{Utilisateur} : compte avec rôle, niveau académique et présence en ligne.
  \item \textbf{Module} : unité pédagogique regroupant composants, niveau, crédits et catégorie.
  \item \textbf{Affectation de formateur} : association entre formateur et module, permettant de distinguer formateurs principaux et simples.
  \item \textbf{Inscription étudiante} : association entre étudiant et module, avec statut d'inscription.
  \item \textbf{Ressource pédagogique} : document ou fichier attaché à un module, enrichi d'un type, d'un auteur et d'un état de validation.
  \item \textbf{Communauté} : espace collaboratif lié à des modules, enrichi de membres et de flux de discussion.
  \item \textbf{Conversation} et \textbf{Message} : support de communication synchrone/asynchrone entre utilisateurs.
  \item \textbf{Progression et badges} : suivi des actions, accumulation de points d'expérience et attribution de récompenses.
\end{itemize}

Ce MCD se traduit par des cardinalités classiques : un module peut être associé à plusieurs formateurs et à plusieurs étudiants, un utilisateur peut publier des ressources, participer à plusieurs conversations et rejoindre des communautés, et chaque communauté regroupe plusieurs membres.

\subsection{Modèle logique des données (MLD)}

Le passage du MCD au MLD normalisé a été réalisé en conservant l'intégrité des relations tout en évitant les redondances. Les données sont organisées sous forme de tables relationnelles, avec des clés primaires auto-incrémentées et des clés étrangères pour garantir les associations.
...
\begin{itemize}
  ...
  \item \texttt{notifications}, \texttt{password\_reset\_tokens}, \texttt{categories} et \texttt{xp\_rules} comme tables de support pour la configuration, les alertes et la sécurité.
\end{itemize}

La table \texttt{student\_enrollments\_view} est une vue matérialisée de consultation des inscriptions, facilitant les requêtes de récupération de parcours sans dupliquer les données.
...
\caption{Dictionnaire de données principal}\label{tab:dictionnaire\_donnees}\\
\hline
\rowcolor{ustoblue}
